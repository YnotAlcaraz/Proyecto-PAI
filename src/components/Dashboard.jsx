  ...
  export const Dashboard = () => {
    ...
    const getEmpleadosMasVentas = () => {
      const empleadosVentas = {};
    
      ventas.forEach((venta) => {
        if (empleadosVentas[venta.empleado]) {
          empleadosVentas[venta.empleado] += 1;
        } else {
          empleadosVentas[venta.empleado] = 1;
        }
      });
    
      const empleadosMasVentas = Object.entries(empleadosVentas).map(([empleadoId, cantidadVentas]) => {
        const empleado = empleados.find((e) => e.id === parseInt(empleadoId));
        return {
          id: empleadoId,
          nombre: empleado ? `${empleado.nombre} ${empleado.apellido_paterno} ${empleado.apellido_materno}` : '',
          cantidadVentas,
        };
      });
    
      return empleadosMasVentas.sort((a, b) => b.cantidadVentas - a.cantidadVentas);
    };
  ...
  <Col span={12}>
    <Card title="Empleados con más ventas" bordered={false}>
      <ol>
        {getEmpleadosMasVentas().map((empleado) => (
          <li key={empleado.id}>
            {empleado.nombre} - Cantidad de ventas: {empleado.cantidadVentas}
          </li>
        ))}
      </ol>
    </Card>
