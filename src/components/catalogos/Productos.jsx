...
export const Productos = () => {
  ...
  const onFinish = () => {
    if ( iden && codBarras && nomb && desc && img && precio) {
      ...
      if (!isEdit) {
        setIsLoading(true);
        axios.post(url, {
          id: iden, codigo_de_barras: codBarras, nombre: nomb, descripcion: desc, imagen_del_producto: img, precio_de_venta: precio
        }).then(() => {
          setIsLoading(false);
          setVisible(false);
        }).catch(err => console.error(err));
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text, record) => <img src={text} alt={record.nombre} style={{ maxWidth: "100px" }} />
    },
    {
        ...
        >
          <Row gutter={10}>
            <Col xs={24} sm={24} md={16}>
              ...
              >
                <Input onChange={e => setCodBarras(e.target.value)} value = {codBarras}/>
              </Form.Item>
              <Form.Item
                name="nombre"
                label="Nombre"
